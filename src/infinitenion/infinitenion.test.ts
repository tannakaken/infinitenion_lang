...
test("height of infinitenion", () => {
    ...
    expect(heightInfinitenion(makeRational(1 as Integer, 2 as Integer) as Rational)).toBe(0);
...
test("real of infinitenion", () => {
    ...
    expect(realOfInfinitenion(makeRational(1 as Integer, 2 as Integer) as Rational)).toStrictEqual({
        numerator: 1,
        denominator: 2
    });
    ...
    expect(realOfInfinitenion(addInfinitenion(nthImaginary(1 as NonNegativeInteger), 1))).toBe(1);
    expect(realOfInfinitenion(addInfinitenion(nthImaginary(1 as NonNegativeInteger), nthImaginary(3 as NonNegativeInteger)))).toStrictEqual(nthImaginary(1 as NonNegativeInteger));
});

test("image of infinitenion", () => {
    expect(imageOfInfinitenion(1)).toBe(0);
    expect(imageOfInfinitenion(1.5)).toBe(0);
    expect(imageOfInfinitenion(makeRational(1 as Integer, 2 as Integer) as Rational)).toBe(0);
    ...
    expect(imageOfInfinitenion(nthImaginary(3 as NonNegativeInteger))).toStrictEqual(nthImaginary(1 as NonNegativeInteger));
    expect(imageOfInfinitenion(addInfinitenion(nthImaginary(1 as NonNegativeInteger), 1))).toBe(1);
    expect(imageOfInfinitenion(addInfinitenion(nthImaginary(1 as NonNegativeInteger), nthImaginary(3 as NonNegativeInteger)))).toStrictEqual(nthImaginary(1 as NonNegativeInteger));
});
...
test("inverse of infinitenion", () => {
    ...
    expect(invInfinitenion(nthImaginary(1 as NonNegativeInteger))).toStrictEqual(negInfinitenion(nthImaginary(1 as NonNegativeInteger)));
    expect(invInfinitenion(0)).toBeNull();
});

test("div infinitenion", () => {
    expect(divInfinitenion(1, 0)).toBeNull();
    expect(divInfinitenion(nthImaginary(3 as NonNegativeInteger), nthImaginary(2 as NonNegativeInteger))).toStrictEqual(nthImaginary(1 as NonNegativeInteger));
});
...
test("power of infinitenion", () => {
    ...
    expect(powInfinitenion(nthImaginary(11111111 as NonNegativeInteger), 12 as Integer)).toBe(1);
});
...
test("test imaginary number", () => {
    ...
    // 可換則のやぶれ
    expect(addInfinitenion(mulInfinitenion(e2, e3), mulInfinitenion(e3, e2))).toBe(0);

    // 八元数
    const e6 = nthImaginary(6 as NonNegativeInteger);
    // 結合則の敗れ
    expect(addInfinitenion(mulInfinitenion(mulInfinitenion(e2, e3), e6), mulInfinitenion(e2, mulInfinitenion(e3, e6)))).toBe(0);
