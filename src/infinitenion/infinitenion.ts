...
export const addInfinitenion = (
  ...
  if (heightA === heightB) {
    ...
    return simplifyCayleyDickson({
      real: addInfinitenion(a2.real, b2.real),
      image: addInfinitenion(a2.image, b2.image),
      height: heightA,
    });
  }
  if (heightA > heightB) {
    const a2 = a as CayleyDickson;
    return simplifyCayleyDickson({
      real: addInfinitenion(a2.real, b),
      image: a2.image,
      height: heightA,
    });
  }
  const b2 = b as CayleyDickson;
  return simplifyCayleyDickson({
    real: addInfinitenion(a, b2.real),
    image: b2.image,
    height: heightB,
  });
};
...
export const negInfinitenion = (a: Infinitenion): Infinitenion => {
  ...
  return {
    real: negInfinitenion(a.real),
    image: negInfinitenion(a.image),
    height: a.height,
  };
};
...
export const mulInfinitenion = (
  ...
  if (heightA === heightB) {
    ...
    return simplifyCayleyDickson({
      real: subInfinitenion(c, d),
      image: addInfinitenion(e, f),
      height: heightA,
    });
  }
  if (heightA > heightB) {
    ...
    const a2 = a as CayleyDickson;
    return simplifyCayleyDickson({
      real: mulInfinitenion(a2.real, b),
      image: mulInfinitenion(a2.image, conjInfinitenion(b)),
      height: heightA,
    });
  }
  const b2 = b as CayleyDickson;
  ...
  // の掛け算の順序に注意
  return simplifyCayleyDickson({
    real: mulInfinitenion(a, b2.real),
    image: mulInfinitenion(b2.image, a),
    height: heightB,
  });
};
...
export const squareNormInfinitenion = (a: Infinitenion): Base => {
  ...
  return addBase(
    squareNormInfinitenion(a.real),
    squareNormInfinitenion(a.image)
  );
};
...
export const infinitenionToString = (a: Infinitenion, nth = 0): string => {
  ...
  return `${infinitenionToString(a.real, nth)} ${infinitenionToString(
    a.image,
    nth + offset
  )} +`;
};
